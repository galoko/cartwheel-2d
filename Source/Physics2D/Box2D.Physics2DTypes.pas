...
const
   ...
   b2_linearSlop = 0.005 / 10;	// 0.5 cm

   /// A small angle used as a collision and constraint tolerance. Usually it is
   /// chosen to be numerically significant, but visually insignificant.
   b2_angularSlop = (2.0 / 10) / 180.0 * Pi;			// 2 degrees
   ...
   b2_linearSleepTolerance = 0.01 / 10;		// 1 cm/s

   /// A body cannot sleep if its angular velocity is above this tolerance.
   b2_angularSleepTolerance = (2.0 / 10) / 180.0;		// 2 degrees/s
...
begin
   Result := Box2D.Physics2DTypes.IsValid(v.x) and Box2D.Physics2DTypes.IsValid(v.y);
end;
...
begin
   Result := Box2D.Physics2DTypes.IsValid(v.x) and Box2D.Physics2DTypes.IsValid(v.y) and
      Box2D.Physics2DTypes.IsValid(v.z);
end;
...
begin
   Result := Box2D.Physics2DTypes.IsValid(x) and Box2D.Physics2DTypes.IsValid(y);
end;
...
begin
   Result := Box2D.Physics2DTypes.IsValid(x) and Box2D.Physics2DTypes.IsValid(y) and
      Box2D.Physics2DTypes.IsValid(z);
end;
