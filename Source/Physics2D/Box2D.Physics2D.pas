...
function b2DefaultMixFriction(friction1, friction2: PhysicsFloat): PhysicsFloat;
function b2DefaultMixRestitution(restitution1, restitution2: PhysicsFloat): PhysicsFloat;

var
  b2MixFriction: function (friction1, friction2: PhysicsFloat): PhysicsFloat = b2DefaultMixFriction;
  b2MixRestitution: function (restitution1, restitution2: PhysicsFloat): PhysicsFloat = b2DefaultMixRestitution;
...
begin
   ...
   begin
      ...
      Result := (d.x >= 0.0) and (d.y >= 0.0) and
         Box2D.Physics2DTypes.IsValid(upperBound) and Box2D.Physics2DTypes.IsValid(lowerBound);
   end;
end;
...
begin
   case m_count of
      ...
      1: Result := 0.0;
      2: Result := Box2D.Physics2DTypes.b2Distance(m_v1.w, m_v2.w);
      3: Result :=
         {$IFDEF OP_OVERLOAD}
...
begin
   ...
   begin
      ...
      distance := Box2D.Physics2DTypes.b2Distance(pointA, pointB);
      iterations := iter;
   end;
...
begin
   ...
   begin
      ...
      begin
         ...
         Box2D.Physics2D.Initialize(worldManifold, manifold^, xfA, xfB, pc^.radiusA, pc^.radiusB);
         {$ENDIF}
   ...
   for i := 0 to m_bodyCount - 1 do
      ...
      begin
         ...
         begin
            ...
            {$ELSE}
            AddBy(v, Multiply(Box2D.Physics2DTypes.Add(Multiply(gravity, m_gravityScale), Multiply(m_force, m_invMass)), h));
            {$ENDIF}
...
begin
   ...
   m_xf.p := m_sweep.c - b2Mul(m_xf.q, m_sweep.localCenter);
   {$ELSE}
   Box2D.Physics2DTypes.Advance(m_sweep, alpha);
   m_sweep.c := m_sweep.c0;
   m_sweep.a := m_sweep.a0;
   SetAngle(m_xf.q, m_sweep.a);
   m_xf.p := Subtract(m_sweep.c, b2Mul(m_xf.q, m_sweep.localCenter));
...
const
   ...
   k_relativeTol = 0.98 / 10;
   k_absoluteTol = 0.001 / 10;
var
   i: Integer;
...
begin
   ...
   {$ELSE}
   length := Box2D.Physics2DTypes.LengthVec(Subtract(anchorB, anchorA));
   {$ENDIF}
end;
...
begin
   ...
   if length > b2_linearSlop then
      ...
      m_u := b2Vec2_Zero;
   {$ELSE}
   length := Box2D.Physics2DTypes.LengthVec(m_u);
   if length > b2_linearSlop then
      DivideBy(m_u, length)
   else
      m_u := b2Vec2_Zero;
...
begin
   ...
   for i := 0 to count2 - 1 do
      m_Ls[i] := Box2D.Physics2DTypes.b2Distance(m_ps[i], m_ps[i+1]);
