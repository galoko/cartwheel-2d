...
uses
  ...
  Cartwheel.CharacterDescription, Cartwheel.Character, Cartwheel.World, Cartwheel.Controllers, Cartwheel.WorldOracle,
  Cartwheel.ControllerInstances, Cartwheel.TwoLinkIK;
...
type
  ...
  public
    ...
    walk: TIKVMCController;
    run: TSimBiController;

    // controls
    VkRightIsPressed, VkLeftIsPressed: Boolean;
    PushTime, PushForce: Double;
    ...
    // debug
    RootAngle, HipAngle, KneeAngle: Double;

    procedure CreateFloor;
    procedure CreateBox(Num: Integer);
    procedure CreateCharacter(X, Y: Double);

    procedure ApplyPushForce(Dt: Double);

    procedure SetupCamera;

    // paint
    procedure BeginPaint;
    procedure DrawDebugVector(const V: TVector2; Color: TAlphaColor; Width: Double = 35);
...
begin
  ...
  'R':
  begin
    CartwheelWorld.removeController(walk);
    CartwheelWorld.removeController(run);
    CartwheelWorld.addController(run);
    run.phi:= walk.phi;
  end;
  'W':
  begin
    CartwheelWorld.removeController(run);
    CartwheelWorld.removeController(walk);
    CartwheelWorld.addController(walk);
    walk.phi:= run.phi;
  end;
  end;
end;
...
begin
  ...
  edge.SetVertices(MakeVector(-1000.0, 0.0), MakeVector(1000.0, 0.0));
...
procedure TMainForm.CreateBox(Num: Integer);
const
  Size = 0.1;
  PosX = 3.0;
  PosY = -0.06;
...
begin
  ...
  fd.friction:= 1.8;
  fd.restitution:= 0.35;
  bd.position:= TVector2.From(PosX + Num * 0.2, PosY + Num * 0.02);
  ...
  MassData.mass:= mass;
  MassData.I:= (Size * Size + Size * Size) * (1/12) * mass * moiScale;
  MassData.center:= TVector2.From(0, 0);
  instance.SetMassData(MassData);

  // instance.SetLinearVelocity(TVector2.From(-9.0, -9.0));
end;


procedure TMainForm.CreateCharacter(X, Y: Double);
var
...
begin
  ...
  character:= desc.createCharacter(World, TVector2.From(X, Y));
  CartwheelWorld.addArticulatedFigure(character);

  desc.Free;

  walk:= TEditableWalking.Create(character);

  worldOracle:= TWorldOracle.Create;
  worldOracle.initializeWorld(world);

  behaviour:= TBehaviourController.Create(character, walk, worldOracle);
  behaviour.initializeDefaultParameters;

  behaviour.requestVelocities(2.5);
  behaviour.requestStepTime(0.59);

  walk.setBehaviour(behaviour);

  behaviour.conTransitionPlan;

  run:= TRunning.Create(character);

  CartwheelWorld.addController(walk);
  // CartwheelWorld.addController(run);
end;

function PointToRect(const P: TPointF; Width: Double) : TRectF;
begin
...
procedure TMainForm.ApplyPushForce(Dt: Double);
var
...
begin
  ...
  begin
    ...
    torso:= character.getARBByName('torso');
    ImpulsePoint:= torso.getWorldCoordinatesPoint(TVector2.From(torso.Width * -Sign(PushForce) / 2, torso.Height / 2));
    torso.Body.ApplyForce(Impulse, ImpulsePoint);
...
begin
  Exit;
  SceneZero.X:= ClientWidth / Scale / 2 - character.getCOM.x;
end;
...
procedure TMainForm.DrawDebugVector(const V: TVector2; Color: TAlphaColor; Width: Double);
begin
  {$IFDEF DRAW_DEBUG}
  Scene.StrokeThickness:= 2.0 / (300 / Scale);
...
var
  ...
  CollisionPoint: TContactPoint;
  TimeDelay: Double;
begin
  Done:= False;
  TimeDelay:= GetRawReferenceTime - LastStepTime;
  if TimeDelay < Step then
  begin
    // WriteLn('too fast');
    Exit;
  end
  else
  if TimeDelay > Step * 1.5 then
    WriteLN(Format('too slow (%f ms)', [TimeDelay - Step]));
  ...
  // DrawDebugVector(TWorldOracle.DebugPoint1, TAlphaColorRec.Blue, 12);
  // DrawDebugVector(TWorldOracle.DebugPoint2, TAlphaColorRec.Blue, 15);
  {
  for CollisionPoint in character.Contacts do
    DrawDebugVector(CollisionPoint.cp, TAlphaColorRec.Red, 14);

  DrawDebugVector(controller.feetMidpoint, TAlphaColorRec.Darkgreen, 25);
  DrawDebugVector(controller.comPosition, TAlphaColorRec.Darkgreen, 25);
  }
  DrawDebugVector(walk.armsTarget, TAlphaColorRec.Darkgreen, 25);
...
begin
  ...
  Scene.Clear(TAlphaColorRec.White);
end;
...
var
  OldScale: Double;
  ScalePoint, MousePoint, OldSize, NewSize, OldMouse, NewMouse: TVector2;
begin
  OldScale:= Scale;
  ...
  ScalePoint:= TVector2.From(ClientWidth, -ClientHeight) / 2;
  OldSize:= ScalePoint / OldScale;
  NewSize:= ScalePoint / Scale;

  MousePoint:= TVector2.From(MousePos.X, -MousePos.Y) - ScalePoint;
  OldMouse:= MousePoint / OldScale;
  NewMouse:= MousePoint / Scale;

  SceneZero:= SceneZero + (NewSize - OldSize) + (NewMouse - OldMouse);
  UpdateMouse;
end;
...
begin
  ...
  begin
    walk.armsTarget:= ToBox2D(MousePos);
    {
    hip:= ikvmcontroller.swingHipIndex;
    knee:= ikvmcontroller.swingKneeIndex;

    RootAngle:= ikvmcontroller.character.getARBByName('pelvis').GetOrientation;

		ikvmcontroller.DebugComputeIKSwingLegTargets(ToBox2D(MousePos));

    HipAngle:= ikvmcontroller.desiredPose.getJointRelativeOrientation(hip);
    KneeAngle:= ikvmcontroller.desiredPose.getJointRelativeOrientation(knee);

    character.Joints[hip].child.SetOrientation(RootAngle + HipAngle);
    character.Joints[knee].child.SetOrientation(RootAngle + HipAngle + KneeAngle);
...
begin
  ...
  MainForm.Scene.StrokeThickness:= 2.0 / (300 / MainForm.Scale);
  // MainForm.Scene.Stroke.Color:= ToBGRA(color);
  MainForm.Scene.Stroke.Color:= TAlphaColorRec.Black;
