...
type
  ...
  public
    ...
    function getJointRelativeOrientation(jIndex: Integer) : Double;
    procedure setJointRelativeOrientation(a: Double; jIndex: Integer);

    function getJointRelativeAngVelocity(jIndex: Integer) : Double;
    procedure setJointRelativeAngVelocity(w: Double; jIndex: Integer);
  end;
  ...
  public
    ...
    procedure setBaseTrajectory(traj: TTrajectory1d);
    function getBaseTrajectory : TTrajectory1d;

    procedure setVTrajScale(traj: TTrajectory1d);
    function getVTrajScale : TTrajectory1d;

    procedure setDTrajScale(traj: TTrajectory1d);
    function getDTrajScale : TTrajectory1d;
  ...
  public
    ...
    procedure setStrengthTrajectory(traj: TTrajectory1d);
    function getStrengthTrajectory : TTrajectory1d;

    procedure setRelativeToCharacterFrame(rel: Boolean = true);
    function isRelativeToCharacterFrame : Boolean;

    procedure addTrajectoryComponent(trajComp_disown: TTrajectoryComponent);
    procedure clearTrajectoryComponents;
    function getTrajectoryComponent(index: Integer) : TTrajectoryComponent;
    function getTrajectoryComponentCount : Integer;

    function evaluateTrajectory(con: TSimBiController; j: TJoint; stance: Integer; phi: Double; const d, v: TVector2;
      bareTrajectory: Boolean = false) : Double;
    function evaluateStrength(phiToUse: Double) : Double;

    function getJointIndex(stance: Integer) : Integer;
  end;
  ...
  public
    ...
    constructor Create;
    function getStateStance(oldStance: Integer) : Integer;

    function getTrajectoryCount : Integer;
    function getTrajectory(idx: Integer) : TTrajectory; overload;
    function getTrajectory(const name: String) : TTrajectory; overload;

    function needTransition(phi, swingFootVerticalForce, stanceFootVerticalForce: Double) : Boolean;
    procedure setNextStateIndex(nextStateIndex: Integer);
    procedure setTransitionOnFootContact(transition: Boolean = true);
    function getTransitionOnFootContact : Boolean;

    procedure setStance(stanceType: Integer);
    function getStance : Integer;

    property Duration : Double read stateTime write stateTime;

    procedure addTrajectory(traj_disown: TTrajectory);
    procedure clearTrajectories;
  end;

  TSimBiController = class(TPoseController)
  public
    lFoot, rFoot, root: TArticulatedRigidBody;
    lHipIndex, rHipIndex: Integer;
    ...
    procedure addControlParams(params: TControlParams); override;

    function getCurrentState : TSimBiConState;
    function getPhiPtr : PDouble;
    ...
    function getState(idx: Integer) : TSimBiConState;
    function getStateCount : Integer;
    procedure addState(state_disown: TSimBiConState);
    ...
    function getStanceFootPos : TVector2;
    function getSwingFootPos : TVector2;
    function getFSMState : Integer;
    procedure updateDAndV;
    procedure computeD0(phi: Double; var d0: TVector2);
    procedure computeV0(phi: Double; var v0: TVector2);
  ...
  public
    ...
    procedure setVelocities(velDS: Double);
    ...
    procedure requestStepTime(stepTime: Double);
    procedure requestStepHeight(stepHeight: Double);
    procedure requestVelocities(velDS: Double);
    procedure requestUpperBodyPose(leanS: Double);
    procedure requestKneeBend(kb: Double);
    ...
    function getPanicLevel : Double;
  end;

  TIKVMCController = class(TSimBiController)
  public
    const
      swingLegPlaneOfRotation = 1;
    var
      ...
      velDSagittal: Double;
      swingFootTrajectorySagittal, swingFootHeightTrajectory: TTrajectory1d;
    ...
    procedure setBehaviour(behaviour_disown: TBehaviourController);
...
begin
	...
	computeIKQandW(swingHipIndex, swingKneeIndex, parentAxis, swingLegPlaneOfRotation, 1, childAxis, pNow, true, pFuture, dt);
end;
...
begin
	...
	computeIKQandW(swingHipIndex, swingKneeIndex, parentAxis, swingLegPlaneOfRotation, 1, childAxis, pNow, False, pFuture, 0.001);
end;
...
begin
	...
	if doubleStanceMode and (cfs.Count > 0) then
...
procedure boundToRange(var v: Double; min, max: Double);
begin
	if v < min then
		v:= min
  else
...
begin
  ...
  begin
		errV:= doubleStanceCOMError*-1;
		desA.x:= (-errV.x + comOffsetSagittal) * 10 + (velDSagittal - v.x) * 150;
	end;
...
begin
  ...
  begin
		...
		lowLCon.unplannedForHeight:= wo.getWorldHeightAt(lowLCon.swingFoot.GetPosition + lowLCon.swingFoot.GetVelocity * 0.3) * 1.0;
    foot_sole_y:= lowLCon.swingFoot.getWorldCoordinatesPoint(TVector2.From(0, -lowLCon.swingFoot.Height / 2)).y;
...
function getValueInFuzzyRange(val, minB, minG, maxG, maxB: Double) : Double;
begin
...
begin
	panicEstimate:= getValueInFuzzyRange(lowLCon.v.x, lowLCon.velDSagittal-0.4, lowLCon.velDSagittal-0.3, lowLCon.velDSagittal+0.3, lowLCon.velDSagittal+0.4);
